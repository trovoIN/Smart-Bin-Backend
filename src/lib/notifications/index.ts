...
export class NotificationService {
    ...
    async sendOTP(phone: string, otp: string): Promise<void> {
        ...
        // and to ensure the function doesn't hang
        try {
            await Promise.allSettled([
                providers.SMS.send(phone, message),
                providers.WHATSAPP.send(phone, message),
            ]);
        } catch (error) {
            // Log error but don't throw - OTP was already stored in DB
            console.error('Error sending OTP notifications:', error);
        }
    }
