...
export async function generateAndStoreOTP(request: OTPRequest): Promise<{
    ...
    const demoNumbers = [
        '+919000000000', // Admin (old)
        '+919000000001', // Supervisor (old)
        '+919111111111', // Collector 1 (old)
        '+919111111112', // Collector 2 (old)
        '+919876543210', // GHMC Admin (new)
        '+919876543211', // Ward Supervisor (new)
        '+919988776655', // Ramesh Kumar Collector (new)
        '+919988776644', // Suresh Babu Collector (new)
    ...
    // Check rate limiting - prevent spam (SKIP for demo accounts)
...
export async function verifyOTP(verification: OTPVerification): Promise<boolean> {
    ...
    // Development bypass for demo accounts
    const demoNumbers = [
        '+919000000000', '9000000000', // Admin
        '+919000000001', '9000000001', // Supervisor
        '+919111111111', '9111111111', // Collector 1
        '+919111111112', '9111111112', // Collector 2
        '+919222222200', '9222222200', // Household 1
        '+919222222201', '9222222201', // Household 2
        '+919222222202', '9222222202', // Household 3
        '+919222222203', '9222222203', // Household 4
        '+919222222204', '9222222204', // Household 5
    ];

    if (demoNumbers.includes(phone) && code === '123456') {
        console.log('✅ Development bypass: Demo account with correct OTP');
        return true;
    }

    // Try to find OTP with multiple phone formats
    ...
    console.log('OTP found with exact match:', !!otpRecord);

    // If not found with exact match, try without +91 prefix
    if (!otpRecord && phone.startsWith('+91')) {
        ...
        console.log('OTP found without prefix:', !!otpRecord);
    }

    // If still not found, try with +91 prefix
    if (!otpRecord && !phone.startsWith('+91') && phone.length === 10) {
        ...
        console.log('OTP found with prefix:', !!otpRecord);
    }

    // No OTP found
    if (!otpRecord) {
        console.log('ERROR: No OTP record found for any phone format');
