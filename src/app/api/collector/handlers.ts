...
export const qrResolveHandler = withCollectorAuth(
    async (request: NextRequest, { user }: { user: AuthContext }) => {
        try {
            ...
            const result = await resolveQRCode(qrToken, UserRole.COLLECTOR, undefined, user.userId);
...
export const markCollectionHandler = withCollectorAuth(
    async (request: NextRequest, { user }: { user: AuthContext }) => {
        try {
            ...
            console.log('=== MARK COLLECTION DEBUG ===');
            console.log('Collector ID:', user.userId);
...
export const getPendingPaymentsHandler = withCollectorAuth(
    async (request: NextRequest, { user }: { user: AuthContext }) => {
        try {
            ...
            return successResponse(formattedPayments);
        } catch (error) {
