...
export const getDashboardHandler = withHouseholdAuth(
    async (request: NextRequest, { user }: { user: AuthContext }) => {
        try {
            ...
            console.log('Household Unit ID:', user.userId);
            ...
            // 4. Get Monthly Collection Stats
            const now = new Date();
            ...
            const monthlyCollections = await prisma.collection.count({
                where: {
                    unitId: unit.id,
                    collectedAt: {
                        gte: monthStart,
            ...
            console.log('Recent collections count:', recentCollections.length);
            console.log('Monthly collections count:', monthlyCollections);
            console.log('Recent collections:', recentCollections.map(c => ({
                date: c.date,
                status: c.status,
            })));

            // 5. Construct Response (matching Frontend expectations)
            return successResponse({
                unit: {
                    id: unit.id,
                ...
                monthlyStats: {
                    ...
                    totalKg: monthlyCollections * 2, // 2 kg per collection
                },
 ...
 * Get complete payment information (status + collector details)
 * This is the main endpoint for the payment page
 */
export const getPaymentInfoHandler = withHouseholdAuth(
    async (request: NextRequest, { user }: { user: AuthContext }) => {
        try {
            // Get unit with collector info
            const unit = await prisma.unit.findUnique({
                where: { id: user.userId },
                include: {
                    collector: {
            ...
            if (!unit) {
                return errorResponse('Unit not found', 'UNIT_NOT_FOUND', 404);
            }
            ...
            return successResponse({
                ...
                collector: unit.collector ? {
                    id: unit.collector.id,
                    name: unit.collector.name,
                    phone: unit.collector.phone,
                    upiId: unit.collector.upiId,
                } : null,
            });
        } catch (error) {
            console.error('Get payment info error:', error);
            return errorResponse('Failed to get payment info', 'SERVER_ERROR', 500);
        }
    }
);
