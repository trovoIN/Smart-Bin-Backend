...
export const getOverviewHandler = withAuth(
    async (request: NextRequest, { user }: { user: AuthContext }) => {
        try {
            ...
            const collectionRate = totalUnits > 0
                ? Math.round((collectionStats.collected / totalUnits) * 100)
                : 0;

            // Build response matching OverviewMetrics interface
            const result: Record<string, unknown> = {
                totalHouses: totalUnits,
                coveredToday: collectionStats.collected || 0,
                missedToday: collectionStats.remaining || 0,  // remaining = not yet collected today
