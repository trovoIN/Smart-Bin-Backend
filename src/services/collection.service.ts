...
export async function getCollectionHistory(
    ...
    startDate.setUTCHours(0, 0, 0, 0); // Use UTC to match database timestamps

    console.log('=== GET COLLECTION HISTORY DEBUG ===');
    ...
    console.log('Collections:', collections.map(c => ({
        ...
        dateKey: c.collectedAt.toISOString().split('T')[0],
    })));
    ...
    console.log('Collection map keys:', Array.from(collectionMap.keys()));

    // Generate history for each day
    const history: CollectionHistory[] = [];
    const currentDate = new Date();
    currentDate.setUTCHours(0, 0, 0, 0); // Use UTC to match database timestamps
    ...
    for (let i = 0; i < days; i++) {
        ...
        if (i < 3) { // Log first 3 days for debugging
            console.log(`Day ${i}: ${dateKey} -> ${collection ? 'COLLECTED' : 'MISSED'}`);
        }
