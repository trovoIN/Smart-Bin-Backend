...
const DEFAULT_MONTHLY_FEE = parseInt(
    process.env.DEFAULT_MONTHLY_FEE || '150',
    10
);
...
export async function claimPayment(
    ...
    // MANDATORY PROOF CHECK - Household must provide evidence
    if (!proofUrl && !transactionRef) {
        throw new PaymentError(
            'Payment proof is required. Please provide a screenshot or UTR number.'
        );
    }
...
// ============================================
// QR-BASED PAYMENT VERIFICATION (COLLECTOR)
// ============================================

/**
 * Verify or reject a payment claim using QR code scan
 * 
 * This enforces that collector must scan household QR before verification
 * Prevents remote fraud and ensures collector is physically present
 * 
 * @param collectorId - Collector performing verification
 * @param qrToken - Scanned QR code token
 * @param input - Verification details
 * @returns Updated payment
 */
export async function verifyPaymentWithQR(
    collectorId: number,
    qrToken: string,
    input: PaymentVerifyInput
): Promise<Payment> {
    const { action, rejectionReason } = input;
    ...
    if (!qr) {
        throw new PaymentError('Invalid QR code');
    }
    ...
    if (qr.unit.collectorId !== collectorId) {
        throw new PaymentError(
            'You are not assigned to this house. Cannot verify payment.'
        );
    }

    // 3. Get current month payment for this unit
    const now = new Date();
    ...
    if (!payment) {
        throw new PaymentError('No payment found for this month');
    }

    // 4. Check payment is in CLAIMED status
    if (payment.status !== 'CLAIMED') {
        throw new PaymentError(
            `Cannot verify payment with status: ${payment.status}`
        );
    ...
    if (action === 'CONFIRM') {
        ...
        await logPaymentAction('PAYMENT_VERIFIED_VIA_QR', payment.id, collectorId, {
            qrToken,
            unitId: qr.unit.id,
    ...
    if (action === 'REJECT') {
        ...
        await logPaymentAction('PAYMENT_REJECTED_VIA_QR', payment.id, collectorId, {
            qrToken,
            unitId: qr.unit.id,
            unitNumber: qr.unit.unitNumber,
            rejectionReason,
